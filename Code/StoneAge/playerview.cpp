...
PlayerView::PlayerView(const QColor color, const std::shared_ptr<Player> player, QObject* parent) : QGraphicsScene(parent), m_player(player)
{
    ...
    m_recources.push_back(addText("Amount of workers: " + QString::number(m_player->getWorkerCount())));
...
void PlayerView::updateText(){
    ...
    m_recources[6]->setPlainText("Amount of workers: " + QString::number(m_player->getWorkerCount()));
